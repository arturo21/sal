...
// Capa que va a funcionar como shortcuts de la función system de php. Sólo para servidores Linux
	 ...
	 * 16: bd (devuelve un objeto xSQLART para operaciones con base de datos)
	 ...
	 * 42: readkey leer del teclado y almacenar en una variable
	 * */
class sal{
	protected $llamadas;
	protected $funciones;
	...
	function passParser($cadena){
	 ...
	 function existsCall($strcal){
		//funcion que valida si existe esa llamada a la API
		for($i=0;$i<count($callsArr);$i++){
			if($callsArr[$i]==$strcal){
				return 10;
			}
		}
		return -1;
